...
class TextureModder(BaseModder):
    ...
    def __init__(
        ...
        def geoms_from_body(body_name, patterns):
            geoms = []
        ...
        # all_geom_names = [f"{self.model.geom(i).name}_{i}" for i in range(self.model.ngeom)]
        ...
        if geom_names is None:
            ...
            for bodyid in range(model.nbody):
                ...
                self.geom_names.extend(geoms_from_body(body_name, geom_names))
            ...
            self.geom_names = list(self.geom_names)

        if ignore_geom_names is not None:
            # need to remove the ignored geoms from the geom_names
            for bodyid in range(model.nbody):
                body_name = model.body(bodyid).name
                ignore_ids = geoms_from_body(body_name, ignore_geom_names)
                # remove them from the geom_names
                ignore_names = [f"{model.geom(i).name}" for i in ignore_ids]                
                self.geom_names = [name for name in self.geom_names if name not in ignore_names]

            for geom_name in all_geom_names:
                for pattern in ignore_geom_names:
                    compiled_pattern = re.compile(pattern)
                    if compiled_pattern.match(geom_name) and geom_name in self.geom_names:
                        self.geom_names.remove(geom_name)
                        break
