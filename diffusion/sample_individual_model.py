...
# score_model = VariableScoreNet(marginal_prob_std=marginal_prob_std_fn, input_channels=3)
...
# with logger.Prefix("/lucidxr/lucidxr/alan_debug/diffusion_v0/2025/07/15/13-45-17/MN2"):
#     ckpt = logger.torch_load("checkpoints/latest.pth", map_location="cpu", weights_only=False)
with logger.Prefix("/lucidxr/lucidxr/alan_debug/diffusion_v1/2025/07/15/16-11-33/MN3/old"):
    ckpt = logger.torch_load("checkpoints/latest.pth", map_location="cpu", weights_only=False)
...
samples = pc_sampler(score_model, marginal_prob_std_fn, diffusion_coeff_fn, sample_batch_size, num_steps=num_steps, device=device)
